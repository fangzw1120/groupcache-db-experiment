...
func (server *Server) register(rcvr any, name string, useName bool) error {
	...
	fmt.Printf("Register service: %+v, %+v, %+v, %+v\n", s.typ.Name(), s.rcvr.Type(), s.rcvr.String(), sname)
	if useName {
		sname = name
	}
	if sname == "" {
	...
	fmt.Printf("Register service: %+v, %+v, %+v LoadOrStore\n", s.typ, s.rcvr, sname)
...
func (server *Server) ServeCodec(codec ServerCodec) {
	...
	for {
		...
		fmt.Printf("ServeCodec readRequest and find service %+v, %+v; reqType %+v; req ServiceMethod %+v; req %+v, resp %+v; keepReading %+v\n", service.name,
			service.method, mtype.method.Name, req.ServiceMethod, argv, replyv, keepReading)
		wg.Add(1)
...
func (server *Server) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	fmt.Printf("RPC server ServeHTTP: %+v, %+v\n", req.RemoteAddr, req.RequestURI)
	reqDump, _ := httputil.DumpRequest(req, true)
	fmt.Printf("RPC server ServeHTTP: %+v\n", string(reqDump))
...
func (server *Server) HandleHTTP(rpcPath, debugPath string) {
	fmt.Printf("RPC server HandleHTTP: %+v, %+v\n", rpcPath, debugPath)
